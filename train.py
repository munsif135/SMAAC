...
class TrainAgent(object):
    def __init__(self, agent, env, test_env, device,dn_json_path, dn_ffw, ep_infos):
        ...
        self.agent = agent # Agent class from agent.py
        self.env = env # l2rpn_wcci_2020s
        self.test_env = test_env #l2rpn_wcci_2020s with test=True
        self.dn_json_path = dn_json_path # ./data\\l2rpn_wcci_2020\\json
        self.dn_ffw = dn_ffw # after reading all the json files in ./data\\l2rpn_wcci_2020\\json. results'd (a dict) look like {(17, 0): (864, 290987.9504394531, 290987.9504394531), (17,1):.......}
    ...
    def chronic_priority(self, cid, ffw, step):
        ...
        scale = 2.
        diff_coef = 0.05
        d = self.dn_ffw[(cid, ffw)][0] # dn_ffw = {(17, 0): (864, 290987.9504394531, 290987.9504394531), (17,1):.......}, if cid = 17, ffw = 0 , dn_ffw[(cid, ffw)][0] is 864 
        progress = 1 - np.sqrt(step/m) # 0.9659793091280114
        difficulty = 1 - np.sqrt(d/m) # 0
        score = (progress + diff_coef * difficulty) * scale # 1.9319586182560229
        return score

    def train(self, seed, nb_frame, test_step, train_chronics, valid_chronics, output_dir, model_path, max_ffw):
        '''
        seed : random number
        nb_frame: defaul value is 100000
        test_step: defaul value is 1000
        train_chronics: [17, 240, 494, 737, 990, 1452, 1717, 1942, 2204, 2403, 19, 242, 496, 739, 992, 1454, 1719, 1944, 2206, 2405, 230, 301, 704, 952, 1008, 1306, 1550, 1751, 2110, 2341, 2443, 2689]
        valid_chronics: list(range(2880, 2890))
        output_dir: eg: ./result\\untitled1245'
        model_path: ./result\\untitled1245\\model'
        max_ffw: 26
        '''
        best_score = -100

        # initialize training chronic sampling weights
        train_chronics_ffw = [(cid, fw) for cid in train_chronics for fw in range(max_ffw)] # train_chronics_ffw = [(17,0),  (17, 1),......]
        total_chronic_num = len(train_chronics_ffw) # total_chronic_num =  832
        chronic_records = [0] * total_chronic_num # chronic_records = [0,0,0,0,.....(832 times)....]
        chronic_step_records = [0] * total_chronic_num # chronic_step_records = [0,0,0,0,.....(832 times)....]

        for i in chronic_records: #chronic_records = [0,0,0,0,.....(832 times)....]
            cid, fw = train_chronics_ffw[i] # train_chronics_ffw[i] is always (17, 0)
            chronic_records[i] = self.chronic_priority(cid, fw, 1)
        # chronic_records =  [1.9319586182560229,0,0,0,0,........
   
        # training loop
        while self.agent.update_step < nb_frame: #every time agent.update() is called agent.update_step will be increased by 1
            ...
            self.env.seed(seed)
            obs = self.env.reset()
            if ffw > 0: #train_chronics_ffw[record_idx]  is 301, 22, then ffw is 22
                # fast_forward_chronics: specifies the number of time steps to fast forward. For example, if you want to fast forward the chronics by 10 time steps, you can call env.fast_forward_chronics(10)
            ...
            self.agent.reset(obs) #reseting all the obs
