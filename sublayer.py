...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_head, d_model, d_k, dropout=0): # n_head = 8, d_model = 128, d_k = 32, dropout=0
        super().__init__()

        self.n_head = n_head
        self.d_k = d_k
        self.w_qs = nn.Linear(d_model, n_head * d_k) # query weights, n_head * d_k(output dimensions) = 256, d_model- input dimensions
        self.w_ks = nn.Linear(d_model, n_head * d_k) # key weights
        self.w_vs = nn.Linear(d_model, n_head * d_k) # value weights
