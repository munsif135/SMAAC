...
def read_ffw_json(path, chronics, case): #case = 'wcci', chronics = train_chronics + valid_chronics (DATA_SPLIT['wcci'][0] + DATA_SPLIT['wcci'][1]),  path = './data\\l2rpn_wcci_2020\\json'
    res = {}
    for i in chronics: #chronics = [17,240,494,737,............]
        for j in range(MAX_FFW[case]): #MAX_FFW[case] = 26 
            with open(os.path.join(path, f'{i}_{j}.json'), 'r', encoding='utf-8') as f: # {i}_{j}.json = \17_0.json (at 1st iteration), final path = './data\\l2rpn_wcci_2020'\\json\\17_0.json'
                a = json.load(f) # a = {"dn_played": 864, "donothing_reward": 290987.9504394531, "donothing_nodisc_reward": 290987.9504394531}
                res[(i,j)] = (a['dn_played'], a['donothing_reward'], a['donothing_nodisc_reward'])  # res = {(17, 0): (864, 290987.9504394531, 290987.9504394531)}
            if i >= 2880: break
    return res
...
if __name__ == '__main__':
    ...
    # settings
    model_name = f'{args.name}_{args.seed}' #untitled1245
    print('model name: ', model_name)

    OUTPUT_DIR = './result'
    DATA_DIR = './data'
    output_result_dir = os.path.join(OUTPUT_DIR, model_name)# ./result\\untitled1245'
    model_path = os.path.join(output_result_dir, 'model') # ./result\\untitled1245\\model'
    ...
    env_name = ENV_CASE[args.case] #args.case = wcci
    env_path = os.path.join(DATA_DIR, env_name) #./data\\l2rpn_wcci_2020'
    chronics_path = os.path.join(env_path, 'chronics')#./data\\l2rpn_wcci_2020\\chronics
    train_chronics, valid_chronics, test_chronics = DATA_SPLIT[args.case]
    dn_json_path = os.path.join(env_path, 'json') #./data\\l2rpn_wcci_2020\\json
