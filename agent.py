...
class Agent(BaseAgent):
    def __init__(self, env, **kwargs):
        ...
        #'mask': This is the specific key we're trying to access from the kwargs dictionary. 
        #2: This is the default value that will be returned if the key 'mask' doesn't exist in the kwargs dictionary.
        ...
        self.input_dim = self.converter.n_feature * self.n_history #n_feature = 5, n_history = 6 therefore input_dim = 30
        
        print(f'N: {self.node_num}, O: {self.input_dim}, S: {self.state_dim}, A: {self.action_dim}, ({self.order_dim})') # 
        print(kwargs)
        self.emb = EncoderLayer(self.input_dim, self.state_dim, self.nheads, self.node_num, self.dropout).to(self.device) # state_dim = 128, nheads = 8 , node_num = 177, dropout = 0.
    ...
    def stack_obs(self, obs):
        ...
        obs_vect = torch.FloatTensor(obs_vect).unsqueeze(0) #takes a 1D numerical array (obs_vect), converts it to a PyTorch FloatTensor, and then adds a batch dimension to it(unsqueeze(0))
        ...
        if len(self.stacked_obs) == 0:
            ...
            self.stacked_obs.append(obs_vect)
         #connectivity_matrix() 177x177 conn_mat[i,j] = 0 means the objects id’ed i and j are not connected to the same bus. 1 if it is connected to same(must check docs for more explanation)
        self.adj = (torch.FloatTensor(obs.connectivity_matrix()) + torch.eye(int(obs.dim_topo))).to(self.device) #torch.eye(int(obs.dim_topo))): identity matrix with 177x177
    ...
    def reconnect_line(self, obs):
        ...
        dislines = np.where(obs.line_status == False)[0] #return a array of list of the indexes in which the values are false , basically the lines that are not active or connected
        for i in dislines:
            act = None
            #obs.time_next_maintenance[i] != 0 means that powerline i is not disconnected from the powergrid for maintenance operation at the current time step.
            if obs.time_next_maintenance[i] != 0 and i in self.converter.lonely_lines: #lonely_lines: # lines that doesn't have neither orgin nor extreme. basically the lines that haven't been connected to any substations
                sub_or = self.action_space.line_or_to_subid[i] # orgin substation of the line i
                sub_ex = self.action_space.line_ex_to_subid[i] # extreme substation of the line i
                if obs.time_before_cooldown_sub[sub_or] == 0: #obs.time_before_cooldown_sub: number of time step the substation is unavailable due to “cooldown”. if it is zero we can able do an action on the substation
                    act = self.action_space({'set_bus': {'lines_or_id': [(i, 1)]}}) #set the bus of the orgin of the line to 1
                if obs.time_before_cooldown_sub[sub_ex] == 0:
                    act = self.action_space({'set_bus': {'lines_ex_id': [(i, 1)]}}) #set the bus of the extreme of the line to 1
                if obs.time_before_cooldown_line[i] == 0: #obs.time_before_cooldown_sub: number of time step the line is unavailable due to “cooldown”. if it is zero we can able do an action on the line
                    status = self.action_space.get_change_line_status_vect() #status = array([False, False, False, False, False........]
                    status[i] = True #change the status of i'th line
    ...
    def act(self, obs, reward, done):
        ...
        self.stack_obs(obs)
        is_safe = self.is_safe(obs) # check whether observaed ratio of thermal is greater than danger value 0.9
        self.save = False        
